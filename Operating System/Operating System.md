...
## 가상메모리
프로그램이 메모리에 올라오면 프로세스가된다. 하지만 프로그램이 메모리의 크기보다 크다면 실행할 수 없을까? 이럴때 사용되는 것이 가상메모리이다. 프로그램을 조각내어 구동에 필요한 부분만 메모리에 올리고 필요하지 않은 부분은 보조기억장치에 저장해놓았다가 필요할 때 부르는 것이다. 실제 물리메모리보다 더 큰 메모리가 있는 것처럼 사용되기 때문에 가상메모리라고 불린다. 

때문에 가상메모리에는 물리메모리에 저장된 실제주소와 논리적인 가상주소가 필요하고 이를 매핑하는 기법이 중요하다. 만약 프로그램을 같은 크기의 단위로 나눈다면 이를 페이지, 가변 크기로 나눈다면 세그먼트라고 한다. 각각 단위들을 사상하는 방법을 페이징, 세그먼테이션이라고 한다.
...
## 세그먼테이션
대부분의 프로그램을 들여다보면 주프로그램, 프로시저, 함수 그리고 전역 또는 지역변수와 호출 시 사용되는 스택등으로 이루어져있다. 이렇게 논리적으로 하나하나의 단위들로 구분이 가능한 서로다른 크기들이 모여 전체푸로그램을 구성한다. 이런 단위를 세그먼트라하면, 세그먼트크기대로 사상하는 방법을 세그먼테이션이라고 한다.

세그먼테이션의 방식은 페이징과 거의 같다. 다른점은 페이징의 경우 페이지와 프레임의 크기가 같기 때문에 페이지 테이블에 실제주소가 아닌 페이지 크기로 계산가능한 값이 주어져있다. 이와 달레 세그먼트 테이블의 경우, 세그먼트의 길이와 시작지점의 실제주소가 주어져있다.
![21_12_2018 5_39 pm office lens](https://user-images.githubusercontent.com/24218456/50333082-7d07f080-0547-11e9-8589-736843e250a2.jpg)
*출처: os? oh yes!
